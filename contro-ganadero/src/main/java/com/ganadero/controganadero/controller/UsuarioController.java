...
public class UsuarioController {
    ...
    @PostMapping()
    public ResponseEntity<?> create(@Valid @RequestBody NuevoUsuarioDTO usuarioDTO){
        try {
    ...
    public ResponseEntity<?> retrive(@PathVariable("id") Long id){
        try {
            UsuarioDTO result = service.retrieve(id);
    ...
    public ResponseEntity<?> list(){
        try {
            List <UsuarioDTO> result  = service.list();
            return ResponseEntity.ok().body(result);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e);
        }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@RequestBody UsuarioDTO usuarioDTO, @PathVariable("id") Long id){
        try {
            UsuarioDTO result = service.update(usuarioDTO, id);
            return ResponseEntity.ok().body(result);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(e);
        }
